...
export interface ProjectGrantRepository {
  ...
  grant(projectId: string, principalId: string, role: ProjectRole): Promise<GrantResult>;
  /** 削除したらtrue。存在しないGrantの取消はfalse（冪等）。他Project・他Roleの同名Grantには影響しない。 */
  revoke(projectId: string, principalId: string, role: ProjectRole): Promise<boolean>;
