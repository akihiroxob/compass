...
export const OutcomeDetailPage = () => {
  ...
  return <Shell><main className="narrow"><Link to={intentPath(projectId, intentId)} className="back-link">← Intent詳細</Link>{error ? <ErrorState message={error} /> : !outcome ? <Loading /> : <>
    ...
    {outcome.status === "active" && <div className="action-row"><Link to={outcomePath(projectId, intentId, outcome.id, "/edit")} className="button">タイトル・仮説を編集</Link><button type="button" className="secondary-button danger" aria-expanded={cancel.confirming} onClick={cancel.open}>Outcomeを取消</button></div>}
    {outcome.status === "active" && cancel.confirming && <ReasonPanel action={cancel} title="このOutcomeを取り消しますか？" description="取り消すと編集できず、Activeに戻せません。成功条件を変えたい場合は、新しいOutcomeを登録してやり直せます。" label={<>取消の理由 <span>必須</span></>} required confirmLabel="取り消す" pendingLabel="取消中..." />}
  </>}</main></Shell>;
};
