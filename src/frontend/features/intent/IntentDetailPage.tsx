...
export const IntentDetailPage = () => {
  ...
  return <Shell><main className="narrow"><Link to={`/projects/${projectId}`} className="back-link">← Project詳細</Link>{error ? <ErrorState message={error} /> : !intent ? <Loading /> : <>
    ...
    {intent.status === "active" && <div className="action-row"><Link to={intentPath(projectId, intent.id, "/edit")} className="button">Intentを編集</Link><button type="button" className="secondary-button danger" aria-expanded={abandon.confirming} onClick={abandon.open}>Intentを放棄</button></div>}
    {intent.status === "active" && abandon.confirming && <ReasonPanel action={abandon} title="このIntentを放棄しますか？" description="放棄すると編集できず、Activeに戻せません。ActiveなOutcomeがあれば同時に取り消されます。新しいIntentを登録してやり直せます。" label="放棄の理由（任意）" confirmLabel="放棄する" pendingLabel="放棄中..." />}
  </>}</main></Shell>;
};
