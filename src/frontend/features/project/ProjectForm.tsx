...
export const ProjectForm = ({ initial, heading, submitLabel, pendingLabel, cancelTo, save }: ProjectFormProps) => {
  ...
  return <Shell><main className="narrow"><Link to={cancelTo} className="back-link">← {cancelTo === "/" ? "Project一覧" : "Project詳細"}</Link><div className="page-heading"><div><p className="eyebrow">{heading.eyebrow}</p><h1>{heading.title}</h1></div></div><p className="lede">{heading.lede}</p>{error && <FormErrorSummary error={error} />}<form onSubmit={submit} className="project-form">
