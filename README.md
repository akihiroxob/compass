...
- 未接続: 外部 Runtime による `outcome_confirmed` の取得・Manager の起動、Agent の自律起動。外部 Runtime による Evidence 還流の起動（`record_execution_evidence` はテスト内の呼び出しで確認）、Evaluation の起動（Evaluator は自動起動されず、Evaluation から再計画・Intent 完了への遷移は Task 36。`record_outcome_evaluation` はテスト内の MCP 呼び出しで確認）。Agent / Runtime 用の不透明 Credential（Task 37。現在は trusted-local）。
...
```bash
# Web API（Web UI・外部Runtime・自動検証用。server 起動中。role は strategist / researcher / manager / worker / reviewer / evaluator / runtime。evaluator・runtime の発行画面は Web UI にありません）
